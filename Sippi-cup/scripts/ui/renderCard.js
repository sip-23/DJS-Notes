...
function createCardElement() {
    ...
    element.className = 'min-w-[280px] max-h-[350px] flex flex-col hover:bg-[#b3b3b3] dark:hover:bg-[#65350F] p-5 gap-1 rounded-lg dark:bg-[#282828] bg-white transition-colors cursor-pointer border border-gray-300 dark:border-none';
    return element;
}
...
function buildCardHTML(podcast, podcastGenres, daysSinceUpdate) {
    return `
        <img src="${podcast.image}" alt="${podcast.title}" class="rounded-md mb-4 w-[240px] h-[190px] object-cover">
        <div class="flex items-center justify-between">
            <h3 class="font-semibold dark:text-white text-black truncate">${podcast.title}</h3>
            <div class="flex items-center gap-1">
                <svg class="fill-[#000000] dark:fill-[#b3b3b3]" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12px" height="12px" viewBox="0 -2.5 21 21" version="1.1">
                <title>love [#1489]</title>
                <desc>Created with Sketch.</desc>
                <defs></defs>
                ...
                <span class="font-medium text-black dark:text-white text-[13px]">75</span>  
            </div>
            </div>
            <div class="flex items-center justify-start gap-2">
            <svg class="fill-[#000000] dark:fill-[#b3b3b3]" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12px" height="12px" viewBox="0 0 100.353 100.353" id="Layer_1" version="1.1" xml:space="preserve">
                <g>
            ...
            <span class="text-sm text-gray-700 dark:text-[#b3b3b3] truncate">${podcast.seasons} seasons</span>
        </div>
        <div class="flex flex-wrap items-start gap-1">
            ${podcastGenres.map(genre => 
                `<button class="dark:bg-[#F4F4F4] bg-[#b3b3b3] w-fit h-fit px-1 text-sm dark:text-[#121212] text-[#0000000] truncate">${genre}</button>`
            ).join('')}
        </div>
        <p class="text-sm text-gray-700 dark:text-[#b3b3b3] truncate">Updated ${daysSinceUpdate} days ago</p>
    `;
}
...
function setupEventListeners() {
    ...
    podcastsContainerTwo.addEventListener('click', (event) => {
        const card = event.target.closest('.podcast-card');
        if (card) {
            const podcastId = card.getAttribute('data-id');
            openPodcastModal(podcastId);
